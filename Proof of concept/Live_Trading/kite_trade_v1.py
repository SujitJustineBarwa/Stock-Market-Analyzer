...
class KiteApp:
    ...
    def exit_position(self, tradingsymbol, exchange, quantity, order_type=None, price=None):
        """ Exits the position for the given tradingsymbol.
        :param tradingsymbol: Trading symbol of the instrument.
        :param exchange: Exchange in which instrument is listed (NSE, BSE, NFO, MCX).
        :param quantity: Order quantity
        :param order_type: Order type (MARKET, LIMIT etc.).
        :param price: Order price (applicable only for LIMIT orders)
        :returns: Order ID.
        """
        # Fetch all positions
        positions = self.positions()['net']  # Corrected to call the method properly

        # Find the position to exit
        position_to_exit = None
        for position in positions:
            # Check if position is a dictionary before accessing elements
            if isinstance(position, dict) and position.get('tradingsymbol') == tradingsymbol and position.get('exchange') == exchange:
                position_to_exit = position
                break

        if position_to_exit is None:
            raise ValueError("No matching position found.")

        # Determine the transaction type based on existing position
        transaction_type = self.TRANSACTION_TYPE_SELL if position_to_exit['quantity'] > 0 else self.TRANSACTION_TYPE_BUY
        ...
        if order_type == self.ORDER_TYPE_LIMIT:
            ...
            order_id = self.place_order(
                ...
                quantity=quantity,
                product=position_to_exit['product'],  # Adjusted to use the correct product
